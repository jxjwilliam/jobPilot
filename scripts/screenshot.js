...
async function screenshotTarget(browser, label, baseUrl) {
  ...
  if (loginDelay > 0) {
    ...
    // Dismiss any overlays before showing the window
    await page.evaluate(() => {
      document.querySelectorAll('[role="alert"], .toast, .notification, [class*="overlay"]').forEach((el) => el.remove());
    });
...
async function main() {
  ...
  const headless = !(loginDelay > 0 || delayOverride);
